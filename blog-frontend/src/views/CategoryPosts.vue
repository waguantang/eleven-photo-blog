...
async function load() {
  ...
  try {
    ...
    category.value = cats.data.data.find(c => c.slug === route.params.slug) || null
    if (category.value) {
      const res = await postApi.list({ page: page.value, size: 9, category: category.value.id })
...
<template>
  <div class="max-w-6xl mx-auto px-4 py-8">
    ...
    <div v-else-if="posts.length === 0" class="text-center py-16">
      <p style="font-family: var(--font-display); color: var(--text-secondary);">这个分类下还没有文章。</p>
    </div>
